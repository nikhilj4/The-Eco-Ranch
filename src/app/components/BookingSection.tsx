...
export function BookingSection() {
    ...
    const handleEnquiry = async () => {
        ...
        const itemLines = items.map(i => `• ${i.name} x${i.qty} = Rs ${(i.price * i.qty).toLocaleString('en-IN')}`).join('%0A');
        const preferredDate = date ? date.toLocaleDateString('en-IN') : 'TBD';
        const waMessage = encodeURIComponent(
            `Hi! I'd like to enquire about a booking at The Eco Ranch.\n\nName: ${name}\nPhone: ${phone}\nEmail: ${email}\nPreferred Date: ${preferredDate}\nPreferred Time: ${time || 'TBD'}\n\nSelected:\n${items.map(i => `• ${i.name} x${i.qty} — Rs ${(i.price * i.qty).toLocaleString('en-IN')}`).join('\n')}\n\nEstimated Total: Rs ${totalAmount.toLocaleString('en-IN')}`
        );

        try {
            await fetch('http://localhost:3001/api/send-enquiry', {
                method: 'POST',
        ...
        // Open WhatsApp
        window.open(`https://wa.me/917411889506?text=${waMessage}`, '_blank');
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-xl border border-stone-100 sticky top-24">
                            ...
                            <div className="space-y-4 pt-6 border-t border-stone-100">
                                ...
                                <Button
                                    ...
                                    size="lg"
                                    className="w-full rounded-xl bg-[#25D366] text-white hover:bg-[#1ebe5d] font-bold h-12 mt-4"
                                    disabled={totalAmount === 0 || isSubmitting}
                                >
                                    {isSubmitting ? 'Sending...' : '💬 Send Enquiry'}
                                </Button>
                                <p className="text-xs text-stone-400 text-center mt-2">
                                    We'll confirm via WhatsApp & email
                                </p>
                            </div>
                        </div>
                    </Reveal>
